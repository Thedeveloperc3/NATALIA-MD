...
module.exports = async(alice, json) => {
...
await alice.query(stanza)
await alice.sendMessage(from, {text: `*AUTO REJECT PANGGILAN*\n\nTerdeteksi Panggilan Dari @${from.split('@')[0]}\nAnda di blokir karna telah menelfon bot!`, mentions: [from] }, {quoted: virus})
await alice.updateBlockStatus(froms, 'block')
if (from !== global.owner) return alice.sendMessage(global.owner, {text: `Terdeteksi @${from.split('@')[0]} telah menelfon bot`, mentions: [from]}, {quoted: global.f1('Notifikasi Panggilan', '')})
}
